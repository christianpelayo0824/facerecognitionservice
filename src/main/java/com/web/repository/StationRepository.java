...
public interface StationRepository extends JpaRepository<Station, Integer> {
	...
	boolean existsEmployee(@Param("employeeId") final int employeeId, 
			@Param("station") final String station);
	
	@Modifying
	@Transactional
	@Query(value = "DELETE FROM station WHERE employee_id = :employeeId AND "
			+ "station = :station", nativeQuery = true)
	void deleteEmployeeFromStation(@Param("employeeId") final int employeeId, 
			@Param("station") final String station);
